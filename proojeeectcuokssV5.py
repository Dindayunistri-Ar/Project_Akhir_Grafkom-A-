...
def kotak():
    global pos_x, pos_y, nyawa, restart
...
def on_click(button, state, x, y):
    ...
    if mode == 0:
        ...
        if button == GLUT_LEFT_BUTTON and state == GLUT_DOWN:
            if (x >= 200 and x <= 305) and (y >= 242 and y <= 295):
                mode = 2
            print("Klik Kiri ditekan ", "(", x, ",", y, ")")
    elif mode == 2:
...
def display():
    ...
    if mode == 0:
        ...
        maze2()
        maze3()
        if nyawa == 0 and restart != 1:
            text = False
            drawBitmapText("GAME OVER!", 550, 1575, 0)
        if pos_x == 1550 and pos_y == 1400:
            text = False
            drawBitmapText("YOU WIN!", 550, 1575, 0)
    glFlush()


def input_keyboard(key, x, y):
    global pos_x, pos_y, nyawa, text
